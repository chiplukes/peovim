...
def _guess_size(space_counts: dict[int, int]) -> int:
    ...
    Strategy: find the smallest indentation amount that appears with at least
    10% frequency, then snap it to the nearest standard candidate.

    Special case: if all qualifying sizes are multiples of 4 and the minimum
    is >= 8, it most likely means multi-level 4-space indentation (e.g. only
    8-space lines visible in the scan window), so prefer 4.
    """
    if not space_counts:
        return 4
    ...
    if min_n >= 8 and all(n % 4 == 0 for n, _ in qualifying):
        return 4
    return min(candidates, key=lambda c: (abs(c - min_n), c))
